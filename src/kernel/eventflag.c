  ...
  if (flgcb->state == KS_EXIST) {
    ...
    for (tcb = wait_queue; tcb != NULL; tcb = tcb->next) {
      if ((tcb->waifct == TWFCT_FLG) && (tcb->waiobj == flgid)) {
        if (check_flag(flgcb->flgptn, tcb->waiptn,
                       tcb->wfmode)) {  // 条件成立の確認
          tqueue_remove_entry(&wait_queue,
                              tcb);  // タスクをウェイトキューから外す
          tcb->state = TS_READY;
          tcb->waifct = TWFCT_NON;
          *tcb->p_flgptn = flgcb->flgptn;
          tqueue_add_entry(&ready_queue[tcb->itskpri],
                           tcb);  // タスクをレディキューへつなぐ
          scheduler();            // スケジューラを実行
...
ER tk_wai_flg(ID flgid, UINT waiptn, UINT wfmode, UINT *p_flgptn, TMO tmout) {
  ...
  flgcb = &flgcb_tbl[--flgid];
  if (flgcb->state == KS_EXIST) {
    if (check_flag(flgcb->flgptn, waiptn,
                   wfmode)) {     // 待ち条件が成立している場合
      *p_flgptn = flgcb->flgptn;  // 条件成立時のフラグ値を返す
      ...
      tqueue_remove_top(
          &ready_queue[cur_task->itskpri]);  // タスクをレディキューから外す
      ...
      cur_task->waitim =
          ((tmout == TMO_FEVR) ? tmout
                               : tmout + TIMER_PERIOD);  // 待ち時間を設定
