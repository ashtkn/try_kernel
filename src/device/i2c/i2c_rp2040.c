...
ER dev_i2c_open(UW unit, UINT omode) {
  ...
  if (ll_devcb[unit].init == FALSE) {
    ...
    out_w(I2C_CON(unit),
          I2C_CON_SLAVE_DISABLE | I2C_CON_MASTER_MODE  // Master Mode設定
              | I2C_CON_RESTART_EN | I2C_CON_TX_EMPTY_CTRL | I2C_CON_7BIT_ADR |
              I2C_CON_SPEED_FAST);  // 7bit Address & Fast Speed設定
 ...
 * 通信パラメータ設定
 */
static void set_com_start(UW unit, UW sadr, UW sdat_num, UW rdat_num, UB *sbuf,
                          UB *rbuf) {
